...
ln -s ../bin/* .
ln -s ../gfs/* .
ln -s ../models/stars/sun/* .	
ln -s ../molecules/* .

echo "Setting the model"
cat intensi.header ap00t5777g44377k1odfnew_grevsau98.dat > synthe.mod
...
## CH usar ch enqto nao se determina masseron ou kurucz
    echo "ch..."
    ln -s CH/ch.asc fort.11
     ./rmolecasc.exe >  /dev/null
    rm -f fort.11

## CN usar ax e bx brookek enqto nao se determina entre brookek e kurucz
    echo "cnaxbrookek..."
...
## OH usar new enqto nao se determina entre update e new
    echo "ohnew..."
    ln -s OH/ohnew.asc fort.11
     ./rmolecasc.exe >  /dev/null
    rm -f fort.11
...
cp spec.asc ../libraries/synthetic/stars
# echo "Spectrum copied to Plots folder"
